...
func (b *findOrderBuilder) buildWhereSearch() {
	if b.req.Search != "" {
		b.values = append(
			b.values,
			// ไม่เควรบวกเข้าไปใน string ตรงๆ strings.ToLower(b.)+"%"
			"%"+strings.ToLower(b.req.Search)+"%", // userid
			"%"+strings.ToLower(b.req.Search)+"%", // address
			"%"+strings.ToLower(b.req.Search)+"%", // contact
		)
...
func (b *findOrderBuilder) buildWhereStatus() {
	if b.req.Status != "" {
		b.values = append(
			b.values,
			// ไม่เควรบวกเข้าไปใน string ตรงๆ strings.ToLower(b.)+"%"
			strings.ToLower(b.req.Status), // userid
		)
...
func (b *findOrderBuilder) buildSort() {
	b.values = append(b.values, b.req.OrderBy) //เหตุผลที่ไม่เพิ่ม sort ใน อากิวแม้นเพราะว่า ASC, DESC จะเรียงอากิวเม้นไม่ได้

	// Fix
	// ต้องแปลงเป็น string ก่อนค่อยเรียง
	b.query += fmt.Sprintf(`
...
func (b *findOrderBuilder) buildPaginate() {
	b.values = append(
		b.values,
		(b.req.Page-1)*b.req.Limit, // offset  = page-1* limit
		b.req.Limit,
	)
...
// ---------------- Engineer Missing Function -------------
// Find
func (en *findOrderEngineer) FindOrder() []*orders.Oders {
	...
	if err := en.builder.getDb().Get(&raw, en.builder.getQuery(), en.builder.getValues()...); err != nil {
		...
		return make([]*orders.Oders, 0)
	}

	ordersData := make([]*orders.Oders, 0)
